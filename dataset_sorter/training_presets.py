...
TRAINING_PRESETS = {
    ...
    "flux2_character_lora": {
        ...
        "description": "Character LoRA for Flux 2. Low rank (16), TEs frozen. "
                       "Benefits from INT8 TE quantization on 24 GB.",
        "config": {
            "network_type": "lora",
            "lora_rank": 16,
            "lora_alpha": 8,
            ...
            "speed_asymmetric": True,
            "speed_change_aware": True,
        },
    },
    "chroma_style_lora": {
        "label": "Chroma Style LoRA",
        "description": "Style LoRA for Chroma (T5-only flow model). "
                       "Single text encoder architecture, rank 16-32.",
        "config": {
            "network_type": "lora",
            "lora_rank": 32,
            "lora_alpha": 16,
    ...
    "hidream_lora": {
        "label": "HiDream LoRA",
        "description": "LoRA for HiDream (4 text encoders). Very VRAM-hungry — "
                       "uses fp8 base + INT8 TE quantization. Low rank recommended.",
        "config": {
            "network_type": "lora",
            "lora_rank": 8,
            "lora_alpha": 8,
    ...
    "marmotte_vram_saver": {
        "label": "VRAM Saver (Marmotte)",
        "description": "Ultra-low memory config using Marmotte optimizer. "
                       "Fits large models (SDXL/Flux) on 8-12 GB GPUs.",
        "config": {
            "network_type": "lora",
            "lora_rank": 16,
            "lora_alpha": 8,
