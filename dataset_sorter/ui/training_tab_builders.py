...
class TrainingTabBuildersMixin:
    ...
    def _build_model_tab(self):
        ...
        g2l.addWidget(QLabel("Conv Alpha"), 4, 0)
        self.conv_alpha_spin = QSpinBox()
        self.conv_alpha_spin.setRange(0, 128)
        self.conv_alpha_spin.setValue(0)
        self.conv_alpha_spin.setToolTip("Convolutional layer alpha. Usually conv_rank/2 or equal to conv_rank. 0 = auto (conv_rank/2).")
        g2l.addWidget(self.conv_alpha_spin, 4, 1)

        # LoRA variants (2024-2026 SOTA)
        self.dora_check = QCheckBox("DoRA — Weight-decomposed (ICML 2024, same quality at half rank)")
        g2l.addWidget(self.dora_check, 5, 0, 1, 2)
    ...
    def _build_dataset_tab(self):
        ...
        g3l.addWidget(self.random_crop_check, 0, 0, 1, 2)
        ...
        g3l.addWidget(self.flip_aug_check, 1, 0, 1, 2)

        g3l.addWidget(_param_label("Color Jitter — Brightness", "0 = off, 0.1 typical"), 2, 0)
        self.cj_brightness_spin = QDoubleSpinBox()
        self.cj_brightness_spin.setRange(0, 1.0)
        self.cj_brightness_spin.setDecimals(2)
        self.cj_brightness_spin.setSingleStep(0.05)
        self.cj_brightness_spin.setValue(0.0)
        self.cj_brightness_spin.setToolTip("Random brightness variation. 0.1 = +/-10%. Helps generalization for lighting variation.")
        g3l.addWidget(self.cj_brightness_spin, 2, 1)

        g3l.addWidget(_param_label("Color Jitter — Contrast", "0 = off, 0.1 typical"), 3, 0)
        self.cj_contrast_spin = QDoubleSpinBox()
        self.cj_contrast_spin.setRange(0, 1.0)
        self.cj_contrast_spin.setDecimals(2)
        self.cj_contrast_spin.setSingleStep(0.05)
        self.cj_contrast_spin.setValue(0.0)
        self.cj_contrast_spin.setToolTip("Random contrast variation. 0.1 = +/-10%. Useful for training diverse lighting conditions.")
        g3l.addWidget(self.cj_contrast_spin, 3, 1)

        g3l.addWidget(_param_label("Color Jitter — Saturation", "0 = off, 0.1 typical"), 4, 0)
        self.cj_saturation_spin = QDoubleSpinBox()
        self.cj_saturation_spin.setRange(0, 1.0)
        self.cj_saturation_spin.setDecimals(2)
        self.cj_saturation_spin.setSingleStep(0.05)
        self.cj_saturation_spin.setValue(0.0)
        self.cj_saturation_spin.setToolTip("Random saturation variation. 0.1 = +/-10%. Good for color diversity.")
        g3l.addWidget(self.cj_saturation_spin, 4, 1)

        g3l.addWidget(_param_label("Color Jitter — Hue", "0 = off, max 0.5"), 5, 0)
        self.cj_hue_spin = QDoubleSpinBox()
        self.cj_hue_spin.setRange(0, 0.5)
        self.cj_hue_spin.setDecimals(2)
        self.cj_hue_spin.setSingleStep(0.01)
        self.cj_hue_spin.setValue(0.0)
        self.cj_hue_spin.setToolTip("Random hue shift. Keep small (0.02-0.05) to avoid unrealistic colors. 0 to disable.")
        g3l.addWidget(self.cj_hue_spin, 5, 1)

        g3l.addWidget(_param_label("Random Rotation", "degrees, 0 = off"), 6, 0)
        self.random_rotate_spin = QDoubleSpinBox()
        self.random_rotate_spin.setRange(0, 45.0)
        self.random_rotate_spin.setDecimals(1)
        self.random_rotate_spin.setSingleStep(1.0)
        self.random_rotate_spin.setValue(0.0)
        self.random_rotate_spin.setToolTip("Random rotation range in degrees. 0 to disable. 5-15 typical for natural-looking augmentation.")
        g3l.addWidget(self.random_rotate_spin, 6, 1)
    ...
    def _build_advanced_tab(self):
        ...
        g1l.addWidget(QLabel("Multires Noise Discount"), 3, 0)
        self.multires_noise_spin = QDoubleSpinBox()
        self.multires_noise_spin.setRange(0, 1.0)
        self.multires_noise_spin.setDecimals(2)
        self.multires_noise_spin.setSingleStep(0.05)
        self.multires_noise_spin.setValue(0.0)
        self.multires_noise_spin.setToolTip("Multi-resolution noise pyramid (Kohya-style). 0 to disable, 0.3-0.8 typical. Improves detail learning at multiple scales.")
        g1l.addWidget(self.multires_noise_spin, 3, 1)
        ...
        g1l.addWidget(self.debiased_check, 4, 0, 1, 2)

        # Loss function
        g1l.addWidget(QLabel("Loss Function"), 5, 0)
        self.loss_fn_combo = QComboBox()
        self.loss_fn_combo.addItem("MSE (L2)", "mse")
        self.loss_fn_combo.addItem("Huber (robust)", "huber")
        self.loss_fn_combo.addItem("Smooth L1", "smooth_l1")
        self.loss_fn_combo.setToolTip("Loss function. MSE is standard. Huber/Smooth L1 are robust to outliers — useful for noisy datasets.")
        g1l.addWidget(self.loss_fn_combo, 5, 1)

        g1l.addWidget(QLabel("Huber Delta"), 6, 0)
        self.huber_delta_spin = QDoubleSpinBox()
        self.huber_delta_spin.setRange(0.01, 10.0)
        self.huber_delta_spin.setDecimals(2)
        self.huber_delta_spin.setSingleStep(0.05)
        self.huber_delta_spin.setValue(0.1)
        self.huber_delta_spin.setToolTip("Transition point for Huber/Smooth L1 loss. Smaller = more robust, larger = closer to MSE. Only used when loss is Huber or Smooth L1.")
        g1l.addWidget(self.huber_delta_spin, 6, 1)
