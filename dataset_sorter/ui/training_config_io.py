...
class TrainingConfigIOMixin:
    ...
    def build_config(self) -> TrainingConfig:
        ...
        _ca = self.conv_alpha_spin.value()
        config.conv_alpha = _ca if _ca > 0 else (self.conv_rank_spin.value() // 2 if self.conv_rank_spin.value() > 0 else 0)
        ...
        config.color_jitter_brightness = self.cj_brightness_spin.value()
        config.color_jitter_contrast = self.cj_contrast_spin.value()
        config.color_jitter_saturation = self.cj_saturation_spin.value()
        config.color_jitter_hue = self.cj_hue_spin.value()
        config.random_rotate_degrees = self.random_rotate_spin.value()
        ...
        config.multires_noise_discount = self.multires_noise_spin.value()
        config.loss_fn = self.loss_fn_combo.currentData() or "mse"
        config.huber_delta = self.huber_delta_spin.value()
    ...
    def apply_config(self, config: TrainingConfig):
        ...
        self.conv_alpha_spin.setValue(getattr(config, "conv_alpha", 0))
        ...
        self.cj_brightness_spin.setValue(getattr(config, "color_jitter_brightness", 0.0))
        self.cj_contrast_spin.setValue(getattr(config, "color_jitter_contrast", 0.0))
        self.cj_saturation_spin.setValue(getattr(config, "color_jitter_saturation", 0.0))
        self.cj_hue_spin.setValue(getattr(config, "color_jitter_hue", 0.0))
        self.random_rotate_spin.setValue(getattr(config, "random_rotate_degrees", 0.0))
        ...
        self.multires_noise_spin.setValue(getattr(config, "multires_noise_discount", 0.0))
        _loss_fn = getattr(config, "loss_fn", "mse")
        for i in range(self.loss_fn_combo.count()):
            if self.loss_fn_combo.itemData(i) == _loss_fn:
                self.loss_fn_combo.setCurrentIndex(i)
                break
        self.huber_delta_spin.setValue(getattr(config, "huber_delta", 0.1))
